...
class Trainer(object):
    ...
    def get_train_iterator(
        ...
        # Caspar
        if load_dataset:
            logger.info("loading train-val (in-domain) data for epoch {}".format(epoch))
            self.task.load_dataset(
                'valid',
                epoch=epoch,
                combine=combine,
        ...
        valid_batch_iterator = self.task.get_batch_iterator(
        ...
        self.reset_dummy_batch(valid_batch_iterator.first_batch)
        return batch_iterator, valid_batch_iterator
    ...
    def train_step(self, samples, valid_samples, raise_oom=False):
        ...
        for i, (sample, valid_sample) in enumerate(zip(samples, valid_samples)):
            sample, is_dummy_batch = self._prepare_sample(sample)
            valid_sample, _ = self._prepare_sample(valid_sample)
